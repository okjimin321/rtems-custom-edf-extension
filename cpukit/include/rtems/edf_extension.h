...
void edf_set_execution_time(rtems_tcb* cur, uint64_t execution_t, uint64_t period); // Assign EDF parameters (execution time and deadline) to the TCB

void edf_start_period(rtems_tcb* cur, uint64_t period); // Reset remaining time and update deadline at the start of a new period
#endif